...
func (tm *TrafficMonitor) CheckTraffic() {
	...
	for _, provider := range providerConfig.Providers {
		...
		go func(p config.ProviderData) {
			...
			Tx, err := strconv.Atoi(traffic.Tx)

			if err != nil {
				log.Println(err)
			}
			log.Println("Current TX: ", utils.FormatSize(int64(Tx)))
		}(provider)
	}
	wg.Wait()
	close(resultsChanTraffic)
	for ch := range resultsChanTraffic {
		Tx, err := strconv.Atoi(ch.Tx)

		if err != nil {
			log.Println(err)
		}
		switch {
		case int64(Tx) > ch.Saturation:
			...
			tm.bot.Send(message)
		case int64(Tx) < 100000000:
			textMessage := fmt.Sprintf("❌ El Trafico cayo a <b><i>%s</i></b> en <b><i>%s</i></b> ❌", utils.FormatSize(int64(Tx)), ch.Name)
			message := tgbotapi.NewMessage(config.GroupChatID, textMessage)
			message.ParseMode = "Html"
			tm.bot.Send(message)
		}
